function [c] = consensus_optimisation(c0,candidates,selected,n_select,sinks,sources,Adj,tstep,tfinal,tlimit,def_cnstrnt,fi_max)
    ...
    if strcmp(def_cnstrnt,'fn')                                     % OPT START: If making a fixed number of sink node selections
        ...
        if (n_select - length(selected))>1                          % If there is more than one node to be selected
            ...
            [c] = check_all_sinks(Adj,candidates,selected,sinks,sources,n_select,tstep,tlimit);
        end
    elseif strcmp(def_cnstrnt,'fi') || strcmp(def_cnstrnt,'ft')     % OPT START: If making a variable resource allocation                                                           
